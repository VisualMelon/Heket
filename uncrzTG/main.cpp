...
void UNCRZ_sprite::setAlpha(LPDIRECT3DDEVICE9 dxDevice)
	{ // probably gonna change, but I need to check over all the shaders first (need the same go-over as model sections)
		switch (alphaMode)
		{
		case AM_none:
			dxDevice->SetRenderState(D3DRS_ALPHABLENDENABLE, FALSE);
...
float g_overheadAmbientCoof = 0.4; // coof of g_overheadLightMod
...
void handleUiEvents(uiEventManager* uiem)
{
	uiEvent* uie;
...
void eval()
{
	...
	lights[0]->lightAmbient = lights[0]->lightColMod * g_overheadAmbientCoof;
...
void initUi(LPDIRECT3DDEVICE9 dxDevice)
{
	...
	rect.top = 0;
	rect.bottom = 40;
	tempTex = new uiTexItem(dxDevice, "banner", mainView, vertexDecPCT, "un_shade.fx", "simpleUi", "ui/banner.tga", rect, &mainUiem, &effects, &textures);
	...
	rect.top = 0;
	rect.bottom = 35;
	tempText = new uiTextItem(dxDevice, "bannertext", mainView, rect, &mainUiem, "Silver's Turn", D3DCOLOR_ARGB(255, 0, 0, 128), uiFont);
...
void initLights(LPDIRECT3DDEVICE9 dxDevice)
{
	...
	ld->lightAmbient = D3DXVECTOR4(0.0, g_overheadLightMod, g_overheadLightMod, 1) * g_overheadAmbientCoof; // keep these two the same... but not
